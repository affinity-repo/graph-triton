...
def _fwd_kernel(
    ...
    for block_n_start_idx in range(0, block_n_end, BLOCK_N_SIZE):
        ...
        if N_LOAD_MASK_NEEDED:
            ...
            k = tl.load(
                k_ptrs + block_n_start_idx * k_n_stride, mask=k_ptr_mask, other=0.0
            )
        else:
        ...
        if HAS_MASK:
            ...
            if (
                IS_MATRIX_MASK
            ):  # mask has a matrix shape, we load (BLOCK_M, BLOCK_N) elements
                attention_mask_offs = (
                    attention_mask_offs[None, :]
                    + m_offs[:, None] * attention_mask_m_stride
                )

            if N_LOAD_MASK_NEEDED & (
                not IS_MATRIX_MASK
            ):  # mask has a vector shape + need a load mask
            ...
            if IS_MATRIX_MASK:  # mask has a matrix shape
                if M_LOAD_MASK_NEEDED & (not N_LOAD_MASK_NEEDED):  # load mask on M axis
                    ...
                    not M_LOAD_MASK_NEEDED
                ) & N_LOAD_MASK_NEEDED:  # load mask on N axis
                    attention_mask_ptr_mask = (
                        block_n_offs[None, :] < attention_mask_n_size
                    )
        ...
        if N_LOAD_MASK_NEEDED:
            v_ptr_mask = (
                block_n_offs[:, None] < n_size
            )  # repeated otherwise triton segfault
            v = tl.load(
                v_ptrs + block_n_start_idx * v_k_stride, mask=v_ptr_mask, other=0.0
            )
        else:
...
class Attention(torch.autograd.Function):
    ...
    def forward(
        ...
        assert q.dtype in [
            torch.float16,
            torch.bfloat16,
        ], f"Only float16 and bfloat16 are supported, got {q.dtype}"
        ...
        if attention_mask is not None:
            ...
            assert (
                attention_mask.size(3) == n_size
            ), "Last size of mask must broadcast on QK^t"
        ...
        _fwd_kernel[
            grid
        ](  # can't use name args because of the way autotune is implemented :-(
            head_size,  # heads
            m_size,  # m_size
            n_size,  # n_size
            ...
            *attention_mask.stride()
            if HAS_MASK
            else (0, 0, 0, 0),  # (batch, heads, m_size, size_k)
...
# fa
Attention(q, k, v, o1, 1.0, False, m)
